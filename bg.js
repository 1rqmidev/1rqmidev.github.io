document.addEventListener("DOMContentLoaded", (e) => {
  ...
  // Remove "moved" if scroll back to top
  const containers = document.querySelector(".logos-container");
  const logoSize = 50; // Size of individual logos
  const gap = 5; // Gap between logos
  ...
  function createGrid() {
    ...
    if (window.innerWidth > 600) {
      // For screens above 600px - diagonal coverage
      cols = Math.ceil((window.innerWidth * 2) / (logoSize + gap)) + 2;
      rows = Math.ceil((window.innerHeight * 2) / (logoSize + gap)) + 2;
    } else {
      // For screens 600px or smaller - normal coverage
      cols = Math.ceil((window.innerWidth * 1.5) / (logoSize + gap)) + 2;
      rows = Math.ceil((window.innerHeight * 1.5) / (logoSize + gap)) + 2;
      containers.classList.add("mobile");
    }

    containers.style.width = `${cols * (logoSize + gap)}px`;
    containers.style.height = `${rows * (logoSize + gap)}px`;
